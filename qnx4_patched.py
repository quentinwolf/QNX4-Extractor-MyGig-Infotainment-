# Patched version of dissect.qnxfs.qnx4
# Fixes for bugs in original dissect.qnxfs package:
# 1. Removed invalid signature check on qnx4_xblk
# 2. Added check to prevent negative seek when xblk_num is 0
#
# Reference:
...
class QNX4:
    ...
    def inode(self, inum: int) -> INode4:
        """Return an inode object for the given inode number.

        Args:
            inum: The inode number.
        """
        return INode4(self, inum)

    def get(self, path: str | int, node: INode4 | None = None) -> INode4:
...
class INode4:
    ...
    def listdir(self) -> dict[str, INode4]:
        """Return a directory listing."""
        return dict(self.iterdir())

    def iterdir(self) -> Iterator[tuple[str, INode4]]:
    ...
    def _iter_chain(self) -> Iterator[tuple[int, int]]:
        ...
        while num_extents and xblk_num:  # BUGFIX: Added "and xblk_num" check
            self.fs.fh.seek((xblk_num - 1) * self.fs.block_size)
            xblk = c_qnx4.qnx4_xblk(self.fs.fh)
            
            # BUGFIX: Removed invalid signature check
            # Original code had: if xblk.signature != b"IamXblk"
            # But qnx4_xblk doesn't have a signature attribute
